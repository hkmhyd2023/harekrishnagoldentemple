...
class ChaitanyaMahaPrabhuFeeds extends StatefulWidget {
  const ChaitanyaMahaPrabhuFeeds({super.key});

  @override
  State<ChaitanyaMahaPrabhuFeeds> createState() => _ChaitanyaMahaPrabhuFeedsState();
}

class _ChaitanyaMahaPrabhuFeedsState extends State<ChaitanyaMahaPrabhuFeeds> {
  ...
  Widget build(BuildContext context) {
                                      ...
                                      snapshot.data!.docs[index];
                                        ...
                                        return Card(
                                          ...
                                          child: InkWell(
                                            borderRadius:
                                                BorderRadius.all(Radius.circular(16)),
                                            onTap: () {                                                                              FirebaseFirestore.instance.collection('ChaitanyaMP-Feeds').doc(document.id).update({"Views": FieldValue.increment(1)});
},
                                            child: Container(
                                              child: Column(
                                                crossAxisAlignment:
    ...
    if (_isLiked) {
      FirebaseFirestore.instance.collection('ChaitanyaMP-Feeds').doc(document.id).update({"Likes": FieldValue.increment(-1)});
      setState(() {
        likeCount -= 1;
        isLiked = false;
...
SizedBox(height: 5,),
                                          ...
                                          child: Row(
                                                      ...
                                                      children: [
                                                        Text (""),
                                                        Text(document['Date'], style: TextStyle(fontWeight: 
                                                        FontWeight.w600),)
                                                      ],
                                                    ),
                                        ),
